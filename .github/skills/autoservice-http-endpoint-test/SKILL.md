...
## Execution Contract
- Run HTTP validation with `python scripts/run-local-test-suite.py http` from the repository root.
- Inspect `tests/.artifacts/test-suite-summary.json`; use the sanitized HTTP result to add, fix, or investigate endpoint tests.
- Do not publish raw `.env`, cookies, tokens, host secrets, absolute local paths, or unsanitized HTTPYAC output.
...
- Keep scenario headers explicit with expected status suffix (`(200 expected)`, `(422 expected)`).
- Keep setup blocks explicit and annotate their expected status.
- Remove unused `@Variables`; avoid copy-pasted dead declarations.
- Use runtime-unique suffixes for create/register entities to avoid rerun collisions.
- Keep precondition notes directly above the target scenario header.
- Keep each scenario focused on one endpoint behavior to simplify debugging.
...
5. Validate variable usage, header status clarity, and request consistency.
6. Remove orphaned variable declarations in touched files.
7. If execution is needed, run the Python runner HTTP target and inspect the sanitized report.
8. Report added/updated/removed cases and residual coverage gaps.
