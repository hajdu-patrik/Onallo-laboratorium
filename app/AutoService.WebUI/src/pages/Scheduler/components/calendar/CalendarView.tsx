...
const CalendarViewComponent = memo(function CalendarView({
  ...
  return (
    <section className="relative select-none overflow-hidden rounded-2xl border border-arsm-border bg-arsm-input p-3 shadow-[0_14px_32px_rgba(28,22,46,0.1),0_0_0_1px_rgba(255,255,255,0.5)_inset] max-[320px]:p-2.5 dark:border-arsm-border-dark dark:bg-arsm-card-dark dark:shadow-[0_20px_42px_rgba(3,5,14,0.6),0_0_0_1px_rgba(255,255,255,0.04)_inset] sm:p-4">
      <div
        aria-hidden="true"
      ...
      <div className="mb-3 flex items-center justify-between max-[320px]:mb-2.5">
        <button
          ...
          className={`rounded-lg border p-1.5 text-arsm-label transition-colors max-[320px]:p-1 dark:text-arsm-label-dark ${canGoPrev ? 'border-arsm-border hover:bg-arsm-accent-subtle hover:text-arsm-accent-deep dark:border-arsm-border-dark dark:hover:bg-arsm-hover-dark dark:hover:text-arsm-primary-dark' : 'cursor-not-allowed border-arsm-border/60 opacity-50 dark:border-arsm-border-dark/60'}`}
        >
        ...
        <button
          ...
          className={`rounded-lg border p-1.5 text-arsm-label transition-colors max-[320px]:p-1 dark:text-arsm-label-dark ${canGoNext ? 'border-arsm-border hover:bg-arsm-accent-subtle hover:text-arsm-accent-deep dark:border-arsm-border-dark dark:hover:bg-arsm-hover-dark dark:hover:text-arsm-primary-dark' : 'cursor-not-allowed border-arsm-border/60 opacity-50 dark:border-arsm-border-dark/60'}`}
        >
        ...
        <>
          ...
          <div className="mb-1 grid grid-cols-7 gap-px max-[320px]:mb-0.5">
            {dayHeaders.map((dayLabel) => (
              <div key={dayLabel} className="py-1 text-center text-[11px] font-semibold uppercase tracking-[0.06em] text-arsm-muted max-[320px]:text-[9px] max-[320px]:tracking-[0.04em] dark:text-arsm-muted-dark">
                {dayLabel}
              </div>
            ))}
          </div>
                    ...
                    const dayContent = (
                      <>
                        <div className="mb-0.5 flex h-7 items-center justify-center max-[320px]:h-6">
                          {day.isToday ? (
                            <span className="flex h-7 w-7 items-center justify-center rounded-full bg-arsm-accent text-sm font-semibold text-arsm-primary shadow-[0_6px_14px_rgba(97,67,154,0.28)] ring-2 ring-arsm-accent/30 max-[320px]:h-6 max-[320px]:w-6 max-[320px]:text-xs dark:bg-arsm-accent-dark dark:text-arsm-hover dark:shadow-[0_8px_16px_rgba(8,10,20,0.5)] dark:ring-arsm-accent-dark/30">
                              {day.date.getDate()}
                            </span>
                          ) : (
                            <span className="text-sm font-medium max-[320px]:text-xs">{day.date.getDate()}</span>
                          )}
                        </div>

                        {/* Mobile indicators: overflow rendered beside the dot */}
                        <div className="mt-0.5 flex h-4 items-center justify-center gap-0.5 leading-none max-[320px]:h-3 md:hidden">
                          {day.appointments.length > 0 ? (
                            day.appointments.slice(0, 1).map((appt) => (
                              <span
                                key={appt.id}
                                className={`flex h-3.5 min-w-3.5 items-center justify-center rounded-full px-1 text-[8px] font-bold leading-none text-arsm-on-accent shadow-sm max-[320px]:h-3 max-[320px]:min-w-3 max-[320px]:px-0.5 max-[320px]:text-[7px] dark:text-arsm-on-accent-dark ${STATUS_DOT_COLORS[appt.status] ?? 'bg-arsm-status-dot-fallback'} ${overflowBadgeToneClass}`}
                            ...
                            <span className="h-3.5 w-3.5 max-[320px]:h-3 max-[320px]:w-3" aria-hidden="true" />
                          )}
                          {day.appointments.length > 1 && (
                            <span className={`text-[8px] font-semibold leading-none text-arsm-muted max-[320px]:text-[7px] dark:text-arsm-muted-dark ${overflowBadgeToneClass}`}>
                              +{day.appointments.length - 1}
                            </span>
                          )}
                        </div>
                        ...
                        <div className="hidden md:flex flex-wrap gap-0.5 justify-center">
                          {day.appointments.slice(0, 3).map((appt) => (
                            <span
                              key={`desktop-${appt.id}`}
                              className={`flex h-5 w-5 items-center justify-center rounded-full text-[9px] font-bold text-arsm-on-accent shadow-sm dark:text-arsm-on-accent-dark ${STATUS_DOT_COLORS[appt.status] ?? 'bg-arsm-status-dot-fallback'} ${overflowBadgeToneClass}`}
                              title={`${appt.vehicle.brand} - ${appt.taskDescription}`}
                            >
                              {appt.vehicle.brand[0]}
                            </span>
