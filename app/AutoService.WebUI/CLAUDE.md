...
- Sidebar layout: main nav at top (default order: scheduler → customers → tools → inventory for non-admin users; admin users get the admin item injected first: admin → scheduler → customers → tools → inventory), bottom section has profile → settings → logout.
...
- `SecuritySection` wires `aria-describedby` to password hint/error text and sets `autoComplete="new-password"`.
...
- `src/store/scheduler.store.ts` — Zustand: `todayAppointments`, `monthAppointments`, `calendarAppointments` (for leading/trailing overflow day cells), `calendarYear`, `calendarMonth`, `selectedDay`, loading states, `upsertAppointment` for optimistic sync (including immediate today/month list updates after intake create when the scheduled date belongs to those views). Scheduler view state (`year`/`month`/`selectedDay`) persists per browser tab in `sessionStorage` key `scheduler-selected-view`.
...
- `src/services/customers/customer-registry.service.ts` — customer registry APIs: list/create/update/delete customers, list/create/update/delete vehicles, and customer/vehicle repair-history queries via `GET /api/customers/{customerId}/appointments` and `GET /api/vehicles/{vehicleId}/appointments` (both support optional `?descending=true`)
...
- `src/types/customers/customers.types.ts` — customer-registry contracts (`CustomerListItem`, customer/vehicle create+update payloads, `VehicleDetailDto`)
...
- `src/pages/Admin/RegisterMechanic/page.tsx` — admin-only mechanic management page (mechanic list with delete + registration form); shows a confirmation modal (`confirmRegisterTitle`/`confirmRegisterMessage`) before submitting registration
...
- `src/pages/Admin/RegisterMechanic/types.ts` — typed form model (`RegisterMechanicFormValues` includes `confirmPassword: string`) and field-error helpers for registration sections
...
- `src/pages/Admin/RegisterMechanic/sections/SecuritySection.tsx` — password and confirm-password inputs with independent show/hide toggles, inline validation state, and accessibility attributes (`autoComplete="new-password"`, `aria-invalid`, `aria-describedby`); confirm-password field is frontend-only (no backend change)
...
- `src/pages/Settings/sections/PersonalInfoSection.tsx` — update first name, middle name, last name, email, phone; applies `aria-invalid` on fields and renders inline per-field errors (including name fields) when validation fails
- `src/pages/Settings/sections/ChangePasswordSection.tsx` — change password with current/new/confirm fields; settings page validates `newPassword.length < 8` client-side before submit (matching admin registration behavior); applies `aria-invalid` on fields when validation errors are present; includes a read-only, visually hidden username autocomplete helper input (`name="username"`, `autoComplete="username"`) wired from `usernameForAutocomplete`
...
- `src/pages/Customers/page.tsx` — protected customers registry page with customer+vehicle CRUD, realtime accent-insensitive name search, clear-X search reset, list sort toggle, and customer/vehicle repair-history panels (each with independent sort toggles)
...
- `src/pages/Scheduler/components/detail/AppointmentDetailModal.footer.tsx` — extracted detail modal footer/status-action section (`AppointmentDetailFooter`) for import-boundary stabilization; status change select has `aria-label` from `scheduler.changeStatus` i18n key; claim CTA is rendered in a full-width centered row in the footer; footer controls respect in-flight mutation locks (`isClaiming`/`isUpdating`)
...
- `src/components/common/Modal.tsx` — reusable dialog shell used by confirmation and crop flows; no X close button — closes via backdrop overlay click or Escape key only; dialog element has a `modal-enter` CSS entrance animation (fade-in + scale-up, 200ms, defined via `@keyframes modal-enter` in `src/index.css`)
...
- `/customers` — protected, CustomersPage inside SidebarLayout
