...
- `Appointment` entity has `DateTime IntakeCreatedAt`, `DateTime DueDateTime`, `DateTime? CompletedAt`, and `DateTime? CanceledAt`; status transitions auto-set/clear the completion/cancel timestamps, while unchanged-status updates are no-op and do not churn those timestamps.
...
- `PUT /api/appointments/{id}/status` (authorized) — update appointment status (assigned mechanic only); idempotent for unchanged status values (no timestamp churn on no-op), auto-sets CompletedAt/CanceledAt on actual status changes, and allows moving Cancelled appointments back to InProgress/Completed (including past-dated appointments)
